...
class MikroTikReadOnly:
    def __init__(self, router: Router):
        ...
        self.client = httpx.AsyncClient(
            ...
            auth=(router.username, password), verify=router.verify_tls, timeout=12,
        )

    async def _read_counter(self) -> tuple[int, int]:
    ...
    async def sample_traffic(self, seconds: float = 2.0) -> dict:
        rx1, tx1 = await self._read_counter()
        started = time.monotonic()
        await asyncio.sleep(seconds)
        rx2, tx2 = await self._read_counter()
...
async def ruijie_status(project_id: str | None) -> dict | None:
    ...
    async with httpx.AsyncClient(timeout=12) as client:
        response = await client.get(
            f"{base}{path}", params={"project_id": project_id},
