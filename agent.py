...
def compose_reply(customer: str, wifi: str, traffic: dict, ruijie: dict | None) -> str:
    ...
    finding = "The connection is currently using a high amount of its download limit." if utilization >= 80 else "The connection is moderately busy right now." if utilization >= 40 else "Traffic is normal right now."
    device = f"\nRuijie/Reyee: {ruijie.get('summary', 'status received')}" if ruijie else ""
    return f"Internet check for {customer} ({wifi})\nDownload: {rx:.2f} Mbps / Upload: {tx:.2f} Mbps\n{finding}{device}\n\nThis was a read-only check. No router settings were changed."
